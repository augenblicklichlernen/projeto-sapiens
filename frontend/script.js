...
let token, userId;
let ytPlayer, lessonView, subjectsGrid;
...
async function updateScores() {
    ...
    if (!sl) return;
    try {
        const res = await fetch(`${API_URL}/api/content/scores/user`, { headers: { Authorization: `Bearer ${token}` } });
        if (!res.ok) {
            if (res.status === 401 || res.status === 403) return forceLogout("Esta conta não possui mais acesso a este site.");
        ...
        if (scores?.length) {
            scores.forEach(s => {
                const p = s.total_lessons > 0 ? ((s.user_score / s.total_lessons) * 100).toFixed(0) : 0;
                sl.innerHTML += `<li><span class="score-name" style="color:${s.color_hex};">${s.name}</span><span class="score-value">${s.user_score}/${s.total_lessons} (${p}%)</span></li>`;
            });
        }
    } catch (err) {
        console.error("Erro ao atualizar scores:", err);
        if (sl) sl.innerHTML = '<li>Erro ao carregar.</li>';
    }
}
...
async function renderQuiz(lesson) {
    fetch(`${API_URL}/api/content/unlock-reinforcement`, {
        ...
        if (d.unlocked) showReinforcementToast(d.title);
    });
    ...
    async function handleAnswer(type, correct, selected, prefix = '') {
        clearInterval(timer);
        qc.querySelectorAll('.option-btn').forEach(b => b.disabled = true);
        qc.querySelector('#confirm-answer-btn').disabled = true;
        const isOk = selected === correct;
        qc.querySelectorAll('.option-btn').forEach(b => { if (b.textContent === correct) b.classList.add('correct'); if (b === selOpt && !isOk) b.classList.add('incorrect'); });

        if (type === 'q1') {
            const fbTxt = prefix + (isOk ? " Correto! Treino." : " Incorreto.");
            const nextBtn = showFeedback(isOk, fbTxt);
            nextBtn.textContent = "Questão 2";
            nextBtn.addEventListener('click', renderQ2);
        } else if (type === 'q2') {
            const res = await fetch(`${API_URL}/api/content/submit-quiz`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ lessonId: lesson.id, isCorrect: isOk, variantIndex: lesson.variantIndex }) });
            const result = await res.json();
            const nextBtn = showFeedback(isOk, prefix + " " + result.message);
            if (result.status === 'completed') { nextBtn.textContent = "Próxima Lição"; nextBtn.addEventListener('click', () => { updateScores(); loadLessons(lesson.subject_id); }); }
            else if (result.status === 'subject_finished') { nextBtn.textContent = "Gerar Certificado"; nextBtn.addEventListener('click', () => handleSubjectFinished(result.subjectId)); }
            else { nextBtn.textContent = "Voltar ao Vídeo"; nextBtn.addEventListener('click', () => renderLessonContent(lesson.id)); }
        }
    }
    async function renderQ2() {
        ...
        if (p.blocked_until && new Date() < new Date(p.blocked_until)) { alert(`Lição bloqueada. Tente em ${new Date(p.blocked_until).toLocaleTimeString()}`); return loadLessons(lesson.subject_id); }
        const seen = p.q2_variants_seen || [];
        const available = [0, 1, 2].filter(i => !seen.includes(i));
        if (available.length === 0) { alert('Variantes esgotadas. Avançando.'); return loadLessons(lesson.subject_id); }
        const vIdx = available[Math.floor(Math.random() * available.length)];
        lesson.variantIndex = vIdx;
        const q = lesson.q2_variants[vIdx];
        const correct = q.options[0];
        qc.innerHTML = `<h3>Questão 2</h3><p>${q.text}</p><div id="timer"></div><div class="options-container"></div><button id="confirm-answer-btn" disabled>Confirmar</button>`;
        const opts = qc.querySelector('.options-container'), timerDisp = qc.querySelector('#timer');
        const onUp = () => handleAnswer('q2', correct, null, 'Tempo esgotado!');
        startTimer(lesson.q2_time, timerDisp, onUp);
        [...q.options].sort(() => .5 - Math.random()).forEach(o => opts.innerHTML += `<button class="option-btn">${o}</button>`);
        selOpt = null;
        opts.querySelectorAll('.option-btn').forEach(b => b.addEventListener('click', () => { if (selOpt) selOpt.classList.remove('selected'); selOpt = b; b.classList.add('selected'); qc.querySelector('#confirm-answer-btn').disabled = false; }));
        qc.querySelector('#confirm-answer-btn').addEventListener('click', () => handleAnswer('q2', correct, selOpt?.textContent));
    }
    function renderQ1() {
        const correct = lesson.q1_options[0];
        qc.innerHTML = `<h3>Questão 1</h3><p>${lesson.q1_text}</p><div id="timer"></div><div class="options-container"></div><button id="confirm-answer-btn" disabled>Confirmar</button>`;
        const opts = qc.querySelector('.options-container'), timerDisp = qc.querySelector('#timer');
        const onUp = () => handleAnswer('q1', correct, null, 'Tempo esgotado!');
        startTimer(lesson.q1_time, timerDisp, onUp);
        [...lesson.q1_options].sort(() => .5 - Math.random()).forEach(o => opts.innerHTML += `<button class="option-btn">${o}</button>`);
        selOpt = null;
        opts.querySelectorAll('.option-btn').forEach(b => b.addEventListener('click', () => { if (selOpt) selOpt.classList.remove('selected'); selOpt = b; b.classList.add('selected'); qc.querySelector('#confirm-answer-btn').disabled = false; }));
        qc.querySelector('#confirm-answer-btn').addEventListener('click', () => handleAnswer('q1', correct, selOpt?.textContent));
    }
    renderQ1();
}
