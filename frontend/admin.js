...
function initializeAdminPanel() { populateSubjects(); populateAllLessonsSelect(); loadManageableSubjects(); loadManageableLessons(); loadManageableUsers(); renderRfQuestionFields(5); }
...
async function populateSubjects() { const select = document.getElementById('select-subject'); if(!select) return; try { const res = await fetch(`${API_URL}/api/content/subjects`); const subjects = await res.json(); if (!res.ok) throw new Error("Erro na API"); select.innerHTML=''; if(subjects.length === 0) { select.innerHTML = '<option value="">Cadastre uma matéria</option>'; return; } subjects.forEach(s => { select.innerHTML += `<option value="${s.id}">${s.name}</option>`}); } catch(e) { select.innerHTML = '<option>Erro</option>'; } }
async function loadManageableSubjects() { const list = document.getElementById('manage-subjects-list'); if(!list) return; try { const res = await fetch(`${API_URL}/api/content/subjects`); const subjects = await res.json(); if (!res.ok) throw new Error("Erro na API"); list.innerHTML=''; if(subjects.length === 0) { list.innerHTML = '<p>Nenhuma matéria.</p>'; return; } subjects.forEach(s => { list.innerHTML += `<div class="manage-item"><span>${s.name}</span><button class="delete-btn" data-id="${s.id}" data-type="subject">Excluir</button></div>`}); } catch(e) { list.innerHTML = 'Erro ao carregar matérias'; } }
async function loadManageableLessons() { const list = document.getElementById('manage-lessons-list'); if(!list) return; try { const subjectsRes = await fetch(`${API_URL}/api/content/subjects`); const subjects = await subjectsRes.json(); if (!subjectsRes.ok) throw new Error("Erro na API"); list.innerHTML=''; let hasLessons = false; for(const subject of subjects) { const lessonsRes = await fetch(`${API_URL}/api/content/lessons/${subject.id}`); const lessons = await lessonsRes.json(); if (lessons.length > 0) hasLessons = true; lessons.forEach(l => { list.innerHTML += `<div class="manage-item"><span><strong>${subject.name}:</strong> ${l.title}</span><button class="delete-btn" data-id="${l.id}" data-type="lesson">Excluir</button></div>`}); } if (!hasLessons) list.innerHTML = '<p>Nenhuma lição.</p>'; } catch(e) { list.innerHTML = 'Erro ao carregar lições'; } }
async function loadManageableUsers() { const list = document.getElementById('manage-users-list'); if(!list) return; try { const res = await fetch(`${API_URL}/api/admin/users`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }); const users = await res.json(); if (!res.ok) throw new Error("Erro na API"); list.innerHTML = ''; if(users.length === 0) { list.innerHTML = '<p>Nenhum usuário.</p>'; return; } users.forEach(u => { list.innerHTML += `<div class="manage-item"><span>${u.username}</span><button class="delete-btn" data-id="${u.id}" data-type="user">Excluir</button></div>`}); } catch(e) { list.innerHTML = 'Erro ao carregar usuários'; } }

async function deleteItem(type, id) { if (!confirm(`TEM CERTEZA? Ação irreversível.`)) return; try { const res = await fetch(`${API_URL}/api/admin/${type}/${id}`, { method: 'DELETE' }); const data = await res.json(); if (!res.ok) throw new Error(data.message); alert(data.message); initializeAdminPanel(); } catch (error) { alert(`Erro: ${error.message}`); } }
...
async function populateAllLessonsSelect() { const select = document.getElementById('select-trigger-lesson'); if(!select) return; try { const subjectsRes = await fetch(`${API_URL}/api/content/subjects`); if (!subjectsRes.ok) throw new Error('Falha ao buscar matérias'); const subjects = await subjectsRes.json(); if (!Array.isArray(subjects)) { console.error("Resposta de /subjects não é um array:", subjects); return; } select.innerHTML = ''; for (const subject of subjects) { const optgroup = document.createElement('optgroup'); optgroup.label = subject.name; const lessonsRes = await fetch(`${API_URL}/api/content/lessons/${subject.id}`); const lessons = await lessonsRes.json(); lessons.forEach(l => { optgroup.innerHTML += `<option value="${l.id}">${l.title}</option>`; }); select.appendChild(optgroup); } } catch (e) { console.error(e); } }
