...
async function populateSubjects() {
    ...
    try {
        ...
        const allSubjects = [...data.main, ...data.extra];
        select.innerHTML = '';
        if (allSubjects.length === 0) {
            select.innerHTML = '<option value="">Cadastre uma matéria primeiro</option>';
            return;
        }
...
async function loadManageableSubjects() {
    ...
    try {
        ...
        const data = await res.json();
        if (!res.ok) throw new Error("Erro na API");
        const allSubjects = [...data.main, ...data.extra].sort((a, b) => a.name.localeCompare(b.name));
        list.innerHTML = '';
        if (allSubjects.length === 0) {
            list.innerHTML = '<p>Nenhuma matéria para gerenciar.</p>';
            return;
        }
        allSubjects.forEach(s => {
            list.innerHTML += `<div class="manage-item"><span>${s.name} ${s.is_extra ? '(Extra)' : ''}</span><button class="delete-btn" data-id="${s.id}" data-type="subject">Excluir</button></div>`;
        });
    } catch (e) {
        list.innerHTML = 'Erro ao carregar matérias';
    }
}
...
async function loadManageableLessons() {
    ...
    try {
        ...
        for (const subject of allSubjects) {
            ...
            lessons.forEach(l => {
                list.innerHTML += `<div class="manage-item"><span><strong>${subject.name}:</strong> ${l.title}</span><button class="delete-btn" data-id="${l.id}" data-type="lesson">Excluir</button></div>`;
            });
        }
        if (!hasLessons) list.innerHTML = '<p>Nenhuma lição para gerenciar.</p>';
    } catch (e) {
        list.innerHTML = 'Erro ao carregar lições';
    }
}
...
async function populateAllLessonsSelect() {
    const select = document.getElementById('select-trigger-lesson');
    if (!select) return;
    try {
        const res = await fetch(`${API_URL}/api/content/subjects`);
        const data = await res.json();
        ...
        for (const subject of allSubjects) {
            ...
            const lessons = await lessonsRes.json();
            lessons.forEach(l => {
                optgroup.innerHTML += `<option value="${l.id}">${l.title}</option>`;
            });
            select.appendChild(optgroup);
        }
    } catch (e) {
        console.error("Erro em populateAllLessonsSelect:", e);
    }
}
